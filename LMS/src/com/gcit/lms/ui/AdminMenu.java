...
//		AdminService ads=new AdminService();
		...
		while(input!=0)
		{
			...
			if((!boookLoans.isEmpty())||boookLoans!=null)
			{
				...
				System.out.println(" Enter number of days you want to extend the DueDate");
				scan=new Scanner(System.in);
				int extendedDays=Integer.parseInt(scan.nextLine());
				...
				c.setTime(dt);
				c.add(Calendar.DATE, extendedDays);   //adding days to the due date

				try {
					brs.changeDueDate(bookId, branchId, cardNo, c.getTime());

					System.out.println("DueDate Successfully Changed to: "+ c.getTime());
				} catch (SQLException e) {
					e.printStackTrace();
				}
...
//		AdminService ads=new AdminService();
		...
		case 1 :
			...
			try {
				lbs.saveLibraryBranch(libraryBranch);
				LibraryBranchOptions();
			} catch (SQLException e) {
				e.printStackTrace();
			}
		...
		case 2:
			...
			try {
				lbs.updateLibraryBranch(libraryBranch);
			} catch (SQLException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
		...
		case 3 :
			...
			try {
				lbs.deleteLibraryBranch(libraryBranch);
			} catch (SQLException e1) {
				e1.printStackTrace();
			}
			LibraryBranchOptions();
